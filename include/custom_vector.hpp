...
class CustomVector {
 public:
  ...
  T pop() { return mInternal[mInternalPtr >= 0 ? mInternalPtr-- : 0]; }

  auto begin() { return mInternal.begin(); }

  auto end() {
    return mInternal.begin() + (mInternalPtr >= 0 ? mInternalPtr : 0);
  }

  void clear() { mInternal.resize(0); };

  const std::vector<T>& getInternal() const { return mInternal; }
